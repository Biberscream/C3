...
int flag_parser(out_flags *flags, int *p_qty, int argc, char **argv,
                char pattern[BUFF][MAX], char files[BUFF][MAX], int *f_qty, int *ef_exist);
...
int processing_row(char *files, int *f_qty, int r, out_flags *flags, char *str,
                   char pattern[BUFF][MAX], int p_qty, int *q_match, int r_count, int *q_non_match);
...
int processing(out_flags *flags,
               char pattern[BUFF][MAX], int *p_qty, char files[BUFF][MAX], int *f_qty) {
...
void *o_param_pnt(out_flags *flags, regmatch_t *pmatch) {
    return (flags->o == 1) ? pmatch : NULL;
}

int i_param(out_flags *flags) {
    return (flags->i == 1) ? 2 : 1;
}

int o_param(out_flags *flags) {
    return (flags->o == 1) ? 1 : 0;
}
...
int flag_parser(out_flags *flags, int *p_qty, int argc, char **argv, char pattern[BUFF][MAX],
    ...
    if (flags->o == 1 && flags->v == 1 && flags->c == 0 && flags->l == 0)
        exit(0);
    return 0;
}
...
int check_struct(out_flags *flags) {
    return (flags->i == 0 && flags->v == 0 && flags->c == 0 && flags->l == 0 && flags->n == 0
        && flags->h == 0 && flags->s == 0 && flags->o == 0) ? 1 : 0;
}
